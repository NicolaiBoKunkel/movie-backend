...
import { migrateMovieSimpleCollection } from "./collections/MovieSimpleCollection";
import { migrateMovieFullCollection } from "./collections/MovieFullCollection";
import { migrateTVShowFullCollection } from "./collections/TVShowFullCollection";
import { migrateActorFullCollection } from "./collections/ActorFullCollection";
...
const migrateFullyNestedMediaItems = async (
  ...
  try {
    ...
    // Migrate all collections using dedicated functions
    const fullyNestedDocs = await migrateMovieSimpleCollection(prisma, connection);
    await migrateMovieFullCollection(fullyNestedDocs, connection);
    await migrateTVShowFullCollection(fullyNestedDocs, connection);
    await migrateActorFullCollection(prisma, connection);
