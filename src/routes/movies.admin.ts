...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM "Movie" WHERE "media_id" = $1`, [id]);
    const delMi = await client.query(
      `DELETE FROM "MediaItem" WHERE "media_id" = $1 AND "media_type" = 'movie'`,
      [id]
    );
    ...
    if (msg.includes('violates foreign key constraint')) {
      return res.status(409).json({ error: 'Cannot delete: referenced by other records', details: msg });
    }
